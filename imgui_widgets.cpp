...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float wheel_thickness = sv_picker_size * 0.08f; // 圆环厚度
    float wheel_r_outer = sv_picker_size * 0.50f; // 圆环外边界
    float wheel_r_inner = wheel_r_outer - wheel_thickness; // 圆环内边界
    ...
    if (flags & ImGuiColorEditFlags_PickerHueWheel)
    {
        ...
        draw_list->PrimVtx(tra, uv_white, 0); // alpha 通道也为0，所以为透明
